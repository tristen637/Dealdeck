    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', fontFamily: 'system-ui, sans-serif' }}>
      <nav style={{ background: '#111827', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {isOnTrial && !isSubscribed && (
            <span style={{ fontSize: 12, color: '#FCD34D' }}>{daysLeft} trial day{daysLeft !== 1 ? 's' : ''} left</span>
          )}
          {!isSubscribed && (
          ...
          {isSubscribed && <span style={{ fontSize: 12, color: '#6EE7B7', fontWeight: 600 }}>✓ Pro</span>}
