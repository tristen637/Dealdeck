...
export default function AuthPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F9FAFB', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: 'system-ui, sans-serif' }}>
      <a href="/" style={{ fontWeight: 800, fontSize: 24, color: '#111827', textDecoration: 'none', marginBottom: 32 }}>Deal<span style={{ color: '#10B981' }}>Desk</span></a>
      <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, padding: 32, width: '100%', maxWidth: 400 }}>
        <h1 style={{ fontWeight: 800, fontSize: 22, marginBottom: 6 }}>{isSignup ? 'Create your account' : 'Welcome back'}</h1>
        <p style={{ color: '#6B7280', fontSize: 14, marginBottom: 24 }}>{isSignup ? 'Start your 7-day free trial — no credit card required' : 'Log in to DealDesk'}</p>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>Email</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="you@example.com" style={{ width: '100%', border: '1px solid #D1D5DB', borderRadius: 8, padding: '10px 12px', fontSize: 14, boxSizing: 'border-box' }} />
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 4 }}>Password</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} required placeholder="min 6 characters" minLength={6} style={{ width: '100%', border: '1px solid #D1D5DB', borderRadius: 8, padding: '10px 12px', fontSize: 14, boxSizing: 'border-box' }} />
          </div>
